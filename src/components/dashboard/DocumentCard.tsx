...
import { FileText, MoreHorizontal, Share2, Trash2, Clock, Users, ChevronRight, Pencil } from "lucide-react"
...
export function DocumentCard({ doc, onDelete, onShare, onRename }: DocumentCardProps) {
  const { user } = useUser()
  ...
  const userRole = doc.members?.[user?.uid || ""] || "VIEWER";
            ...
            <div onClick={(e) => e.preventDefault()}>
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48 p-2 rounded-xl shadow-2xl border-none">
                  {canEdit && (
                    <DropdownMenuItem onClick={() => setIsRenameOpen(true)} className="gap-3 p-3 rounded-lg cursor-pointer">
                      <Pencil className="h-4 w-4" /> Rename Document
                    </DropdownMenuItem>
                  )}
